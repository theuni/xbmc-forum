var MyBB = {
	...
	enrichContent: function(t) {
		t.innerHTML = t.innerHTML.replace(/\bPR([0-9]+)\b/ig , '<a href="https://github.com/xbmc/xbmc/pull/$1" target="_blank">PR:$1</a>');
		t.innerHTML = t.innerHTML.replace(/\b(([a-f0-9]{7})([a-f0-9]{33}))\b(?!([^<]+)?>)/g , '<a href="https://github.com/xbmc/xbmc/commit/$1" target="_blank">Commit:$2</a>');
		//t.innerHTML = t.innerHTML.replace(/\b([a-f0-9]{7})\b(?!(\S+"|\s*<\/a>|[^<]+>))/g , '<a href="https://github.com/xbmc/xbmc/commit/$2" target="_blank">Commit:$2</a>');
	},
	...
	quickLogin: function()
	{
		// only add quicklogin if not already added
		if($("quick_login") && !$("loginForm"))
		{
			var form = new Element("form", { method: "post", action: "member.php", id: "loginForm" });
			form.insert({ bottom: new Element("input",
				{
					name: "action",
					type: "hidden",
