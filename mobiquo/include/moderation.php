...
function m_get_report_post_func($xmlrpc_params){	
	...
	$reports = '';
	$query = $db->query("
		SELECT r.*, u.username, up.username AS postusername, up.uid AS postuid, t.subject AS threadsubject, p.dateline as postdateline, up.avatar, p.message as postmessage, p.subject as postsubject, t.views, t.replies, IF(b.lifted > UNIX_TIMESTAMP(), 1, 0) as isbanned, p.visible
...
function m_get_moderate_topic_func($xmlrpc_params)
{
	...
	$query = $db->query("
		SELECT t.*, p.message AS postmessage, u.avatar, f.name as forumname, IF(b.lifted > UNIX_TIMESTAMP(), 1, 0) as isbanned
...
function m_get_moderate_post_func($xmlrpc_params)
{
	...
	$query = $db->query("
		SELECT p.pid, p.subject, p.message, t.subject AS threadsubject, t.tid, u.username, p.uid, t.fid, p.dateline, u.avatar, t.views, t.replies, IF(b.lifted > UNIX_TIMESTAMP(), 1, 0) as isbanned
		FROM  ".TABLE_PREFIX."posts p
		LEFT JOIN ".TABLE_PREFIX."threads t ON (t.tid=p.tid)
